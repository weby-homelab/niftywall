...
- **🛡️ SAFE Mode (Екстрене блокування):** Кнопка **SAFE Mode** активує режим екстреного захисту для фаєрвола. Це миттєво скидає всі поточні правила та застосовує "стерильну" конфігурацію, дозволяючи трафік лише через довірені порти (наприклад, SSH) та інтерфейси (Tailscale/Local). Це ваш "стоп-кран" під час активної атаки.
